#include <iostream>

#ifndef AES_128_COMMON_H
#define AES_128_COMMON_H

typedef unsigned int byte;
const int BLOCK_SIZE = 16;
const int N_ROUNDS = 10;
const byte IRR_POLYNOMIAL = 0x11B;

const byte S_BOX[] = { 
};

byte *string_to_byte_array(std::string str);

void print_byte_array(byte *byte_array, size_t length);

void print_state(byte *byte_array);

void newline();

void add_round_key(byte *byte_array, const byte *key);

byte substitute_byte(byte byte_to_substitute);

void substitute_bytes(byte *byte_array);

byte gf_multiply(byte a, byte b);

byte msb(byte a);

void matrix_vector_multiply(const byte *A, const byte *b, byte *c, int n);

void clone_byte_array(const byte *byte_array, byte *clone, int len);

std::string byte_array_to_string(const byte *byte_array, int len);
#endif //AES_128_COMMON_H
